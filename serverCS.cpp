...
int main(){
    cout<<"The ServerCS is up and running using UDP on port "<<serverCS_PortNumber<<"."<<endl;
    thread thread_udp(udp_server, serverCS_PortNumber, serverM_UDP_PortNumber);
    thread_udp.detach();
	...
	while(1){	
		if(udp_recv_message != ""){
            ...
            cout<<"The ServerCS receiveed a request from the Main Server about the " + course_category + " of " + course_name + "."<<endl;
...
void split(string file_strs){
    ...
    int start = i + 1;
    for(i; i < file_strs.size(); i++){
        if(file_strs[i] == ','){
            string temp = file_strs.substr(start, i - start + 1);   
